...
/// 当想要【可变字符】或者 
/// 
/// 当并非所有字符串的值都能在编写代码时就知道:
/// 
///             例如, 要是想获取用户输入并存储该怎么办呢? 时用   【String 类】
/// 
...
fn string_own_mv() -> () {
    ...
    let s2 = s1; // move 语义 (基础类型实现了 Copy，而 String 未实现，所以这里是  move 语义)

    println!("{},  world!",  s1); // error[E0382]: borrow of moved value: `s1`. 即 s1 的所有权已经转移到 s2


    // 但是对于 基础类型 不适用 (基础类型 实现了 Copy)
...
/// 
/// 
/// Rust 【不允许】 【自身】 或 【其任何部分(字段)】 实现了 Drop trait 的类型使用 Copy trait.
/// 
/// 
/// 
/// 如果我们对其值离开作用域时需要特殊处理的类型使用 Copy 注解
/// 
/// 1、 任何一组简单   [标量值]  的组合都可以实现 Copy  <栈上的>
/// 
/// 2、 任何 不需要   [分配内存]   或   [某种形式资源]    的类型都可以实现 Copy  <栈上的>
/// 
/// 
/// 
/// 
...
} // 这里, s3 移出作用域并被丢弃 [没有被 return 则将被丢弃]；s2 也移出作用域, 但已被移走, 所以什么也不会发生；s1 离开作用域并被丢弃
...
fn gives_ownership() -> String {             // gives_ownership 会将   返回值移动给   调用它的函数
    ...
    some_string                              // 返回 some_string  并【移出给调用的函数】    所有权 【被移出】 到函数外头了
...
///     {
/// 
///         let r1 = &mut s; // <读写>   r1 类型: &mut
/// 
///     } // r1 在这里离开了作用域, 所以我们完全可以再创建一个新的引用 r2
/// 
///     let r2 = &mut s; // <读写>       r2 类型: &mut
/// 
/// 
/// ********************************************************************
...
/// ********************************************************************
/// 
/// 使用 返回 非引用变量 (将  非引用变量  所有权转移出函数 来解决 悬垂引用)
/// 
/// 
/// fn main() {
...
/// 
/// 
/// 传递 引用不代表 获得变量的所有权,  即: &String 只是 String 的引用, 但没有获取 String 的所有权 (只有 【读】 的权限， 获取所有权是同时具备 【读】和 可能【写】权限),  
/// 而将 String 赋值给其他 String类型变量才是 所有权转移 (注意：mut String 才具备 【写】， String 只获取所有权而无【写】权限)
/// 
/// 
/// 
...
/// 
/// 
///                             【编译器】 使用    三条消除规则   来确定哪些场景 【不需要】 显式地去 【标注】 生命周期
/// 
/// ######################################################################################################################################################
/// ######################################################################################################################################################
///
...
/// **********************************************************************************
/// 
/// 2. 若 【只有】 一个输入生命周期  (函数参数中只有一个引用类型)  ，那么该生命周期会被赋给    所有的输出生命周期   
/// 
/// 
/// 
///                         -------------------      也就是:          所有返回值的生命周期都等于该输入生命周期
/// 
/// 
/// 
/// **********************************************************************************
...
/// **********************************************************************************
/// 
/// 3. 若存在多个输入生命周期，且其中一个是 &self 或 &mut self    
/// 
/// 
///                             -----------------       则 &self 的生命周期被赋给所有的   输出生命周期
/// 
/// 
/// 
/// **********************************************************************************
...
/// 
/// 
///         例如第三条规则，若一个方法，它的返回值的生命周期就是跟参数 &self 的不一样怎么办？总不能强迫我返回的值总是和 &self 活得一样久吧？! 问得好，答案很简单：    
/// 
/// 
///                                     ---------------    手动标注生命周期
/// 
...
///         
///         编译器会自动编成:            fn first_word<'a>(s: &'a str) -> &'a str     -------------  根据 【第一条规则】 和 【第二条规则】
///         
/// 
/// 
/// 又如：
...
/// 以上 3 条 规则  (编译器自动为函数各个 引用参数 <只有引用参数才会被 借用啊，不是转移所有权，所以 引用肯定需要关心生命周期啊> 加 生命周期的 规则) 总结为： 
/// 
/// 
/// 
/// 1. 编译器会为函数的每一个 引用参数加上生命周期参数：                                            fn get_data(id: &'a str, name: &'b str) ->  &'c str 
/// 
/// 2. 编译器会为 单入参 函数 的所有返参加上入参的生命周期参数：                                    fn get_data(id: &'a str) ->  (&'a str, &'a str)
/// 
/// 3. 编译器会为 &self 或 &mut self 方法的 所有所有返参加上入参的生命周期参数：                    fn get_data(&'a self, id: &'b str) ->  (&'a str, &'a str)
/// 
/// 
/// 
/// 
